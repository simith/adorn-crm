...
function normalizeEvent(raw: unknown, fallbackTimestamp: string): SessionEvent {
    ...
    if (typeof payload.jewellery_id === "string" && payload.jewellery_id.trim()) {
        event.jewellery_id = payload.jewellery_id.trim();
    }
...
function sessionsFromPayload(payload: unknown) {
    ...
    if (
        ...
        return (payload as { sessions: unknown[] }).sessions.map((session, index) =>
...
export async function POST(request: Request) {
    ...
    const jewelleryId = typeof raw.jewellery_id === "string" ? raw.jewellery_id.trim() : "";
